...
```
Создайте в корневой папке проекта файл `.env` и укажите в нем настройки доступа к базе данных PostgreSQL в следующем формате - `DATABASE_DSN=driver://user:pass@localhost/dbname`. Для тестирования возможно запустить отдельный контейнер с PostgreSQL. Для этого выполните следующую команду:
```
sudo docker run --rm \
...
```
Так же в файле `.env` есть возможность указать имя для сервиса:
```
